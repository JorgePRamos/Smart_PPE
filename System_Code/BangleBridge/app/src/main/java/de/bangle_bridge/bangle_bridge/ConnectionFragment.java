...
/**
 * The class responsible of the functionality behind the device fragment which shows the BLE devices in range and allows the user to connect to them. Will prompt with permission dialogs if BLE connectivity is not accessible.
 * @author Jorge
 * @version 1.5
 * @since 1.0
 */
public class ConnectionFragment extends ListFragment {
    ...
    public ConnectionFragment() {//Creation from mainActivity
    ...
    public void onCreate(Bundle savedInstanceState) {//Creation of view
        ...
        listAdapter = new ArrayAdapter<BluetoothDevice>(getActivity(), 0, listItems) {
            ...
            public View getView(int position, View view, ViewGroup parent) {//Set devices list.xml as layout if not already is layout in position p
                ...
                if (view == null)
                    view = getActivity().getLayoutInflater().inflate(R.layout.bt_item, parent, false);
                TextView text1 = view.findViewById(R.id.text1);//text1 and text2 in bt_item layout
    ...
    public void onActivityCreated(Bundle savedInstanceState) { // Set header --> deviceListHeader.xml
        ...
        setListAdapter(null);
        View header = getActivity().getLayoutInflater().inflate(R.layout.list_header, null, false);//get xml
    ...
    private void updateScan(BluetoothDevice device) {
        ...
        if(listItems.indexOf(device) < 0) {
            ...
            Collections.sort(listItems, ConnectionFragment::compareTo);//sort fragment by name
            listAdapter.notifyDataSetChanged();
        }
    }
