...
/**
 * Socket class for BLE communication connect, disconnect and write as methods.
 * @author Jorge
 * @version 1.5
 * @since 1.0
 */
class BtSocket extends BluetoothGattCallback {
    ...
    BtSocket(Context context, BluetoothDevice device) {
        if(context instanceof Activity)
            throw new InvalidParameterException("expected non UI context");
        this.context = context;
        this.device = device;
        writeBuffer = new ArrayList<>();//Buff out
    ...
    }//Get name and address if name is not Null
    ...
    void disconnect() {
        ...
        device = null;
        canceled = true;
        //Prepare for disconnect cleaning all Vars
        synchronized (writeBuffer) {//Stop pending writting and clear device buffer
            writePending = false;
            writeBuffer.clear();
        }
        readCharacteristic = null;
        writeCharacteristic = null;
        //Disconnect asynchronous delegate
        if(delegate != null)
            delegate.disconnect();
        if (gatt != null) {
    ...
    //Errors returned asynchronusly to listener
    void connect(BtListener listener) throws IOException {
    ...
    private void onPairingBroadcastReceive(Context context, Intent intent) {
        // for ARM Mbed, Microbit uses Android bluetooth settings

        BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
        if(device==null || !device.equals(this.device))
            return;
        switch (intent.getAction()) {
            case BluetoothDevice.ACTION_PAIRING_REQUEST: //Request of pairing
                ...
                break;
            case BluetoothDevice.ACTION_BOND_STATE_CHANGED: //Action bonded
    ...
    @Override
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {//Gets connection status from gatt and acts depending on it
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {//gatt uid Type
        boolean sync = true;
        writePending = false;
        //creation of delegate depending on type of connection
    ...
    //Deleagte for gatt different UIDs
